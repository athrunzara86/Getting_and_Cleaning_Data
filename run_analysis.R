setwd("C:\\Users\\LG\\Desktop\\UCI HAR Dataset")
library(dplyr)
...
results_final<- rbind(train4,test4)

#Summarizing the data
dataResults<- (results_final%>% group_by(subject,Activity)%>% summarise_each(funs( mean)))
print(dataResults)
                                  
write.table(dataResults,"./dataResults.txt",sep=" ",row.name=FALSE) 